  ...
  _createServiceListen () {
    ...
    this.replySocket.on("message", function () {
      var args = Array.apply(null, arguments)
        ...
        var modulo = request.name.split('__'); // 'primo__2.0.0__ehPrimo' in list
        var name = modulo[0];
        ...
        if (modulo.length === 3) {
          ...
          Object.keys(self._services).forEach((instance) => {
            var m = instance.split('__');
            if (m[0] === name && m[1] > version) {
              version = m[1];
            }
          });
        }
...
commander.option('-p --port <number>', 'specify a port')
  .parse(process.argv);
